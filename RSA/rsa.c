...
long long rsa_modExp(long long b, long long e, long long m)
{
    ...
    return (e % 2 == 0) ? rsa_modExp(b * b % m, e/2, m) % m : b * rsa_modExp(b, (e-1), m) % m;
}
...
void rsa_gen_keys(struct public_key_class *pub, struct private_key_class *priv, const char *PRIME_SOURCE_FILE)
{
    ...
    do{
        ...
        unsigned int a =  (double)rand() * (prime_count + 1) / (RAND_MAX + 1.0);
        unsigned int b =  (double)rand() * (prime_count + 1) / (RAND_MAX + 1.0);
...
long long *rsa_encrypt(const char *message, const size_t message_size, const struct public_key_class *pub)
{
...
char *rsa_decrypt(const long long *message, const size_t message_size, const struct private_key_class *priv)
{
